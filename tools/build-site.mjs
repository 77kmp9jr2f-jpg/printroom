...
for (const name of ['app.js','settings.js','ui-core.js','color-library.js','color-library-view.js','camera.js','style.css']) {
  ...
  if (name === 'camera.js') code = code.replaceAll('NO CAMERA CONFIGURED', 'SIMULATED · ILLUSTRATION');
  if (name === 'app.js') code = code.replaceAll('Local connection', 'Simulated data').replaceAll('LAN connection', 'Simulated data');
...
const banner = `<aside class="demo-banner" aria-label="Simulated demo"><a href="../">← Printroom</a><strong>SIMULATED DEMO</strong><span>No printer or network connection</span><label>Room size <select id="demo-size"><option value="2">2 printers</option><option value="12">12 printers</option><option value="64">64 printers</option></select></label><button id="demo-reset" type="button">Reset demo</button><a href="../installation.html">Install your own ↗</a></aside>`;
...
const profiles = Array.from({length:64},(_,i) => ({id:`demo-${i+1}`, name:i<2?['Studio One','Studio Two'][i]:`Workshop ${String(i+1).padStart(2,'0')}`,host:`192.168.250.${i+10}`,moonrakerPort:7125,fluiddPort:null,adapter:'moonraker',cameraMode:'none',enabled:true,hasApiKey:false}));
