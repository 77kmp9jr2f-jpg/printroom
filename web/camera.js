export class CameraView {
  constructor({ video, container, status, hint, api, printerId, active = true, enabled = true }) {
    ...
    if (active && enabled) this.connect(); else this.label(enabled ? 'CAMERA PAUSED' : 'NO CAMERA CONFIGURED');
  }
  label(state, detail = '') {
  ...
  setActive(active) {
    ...
    else { ++this.generation; clearTimeout(this.retry); this.retry = null; this.peer?.close(); this.video.srcObject = null; this.lastFrameAt = 0; this.label(this.enabled ? 'CAMERA PAUSED' : 'NO CAMERA CONFIGURED'); }
  }
  async connect() {
